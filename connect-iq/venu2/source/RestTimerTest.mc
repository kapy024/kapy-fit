...
(:test)
function testTomaElExtremoAltoDeUnRango(logger as Toybox.Test.Logger) {
...
(:test)
function testSinDescansoNoDaDuracion(logger as Toybox.Test.Logger) {
    Toybox.Test.assertEqual(RestTimer.parseRestSeconds("Sin descanso"), null);
}

(:test)
function testLeeElNumeroConAclaracionEntreParentesis(logger as Toybox.Test.Logger) {
    Toybox.Test.assertEqual(RestTimer.parseRestSeconds("10 seg (entre intervalos)"), 10);
}

(:test)
function testConvierteMinutosASegundos(logger as Toybox.Test.Logger) {
    Toybox.Test.assertEqual(RestTimer.parseRestSeconds("hasta 1 min continuo"), 60);
}

(:test)
function testSinEtiquetaNoDaDuracion(logger as Toybox.Test.Logger) {
    Toybox.Test.assertEqual(RestTimer.parseRestSeconds(null), null);
}

(:test)
function testFormatMMSSRellenaLosSegundos(logger as Toybox.Test.Logger) {
